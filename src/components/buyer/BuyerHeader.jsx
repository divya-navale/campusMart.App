...
import { Home, DollarSign, Clock, Activity, Grid, Filter } from 'lucide-react';
import { RESIDENCE_OPTIONS, PRICE_OPTIONS, CONDITION_OPTIONS, AGE_OPTIONS, CATEGORY_OPTIONS } from '../../constants/options';
import { fetchFilteredProducts } from '../../services/api';  // Import the API function
...
const BuyerHeader = () => {
  ...
  // Fetch filtered products when the filters change
  const handleFilterSubmit = async () => {
    ...
    try {
      const data = await fetchFilteredProducts(filters);  // Call the API function
      setFilteredProducts(data);  // Store the filtered products
      console.log('Filtered Products:', data);  // Log the filtered products to the console
    } catch (error) {
    ...
    <Card className="shadow-sm border-0" style={{ width: '280px' }}>
      ...
      <Card.Footer className="bg-white border-top-0 p-3">
        <Button 
          ...
          onClick={handleFilterSubmit} // Handle the click event here
        >
          Apply Filters
        </Button>
      </Card.Footer>
